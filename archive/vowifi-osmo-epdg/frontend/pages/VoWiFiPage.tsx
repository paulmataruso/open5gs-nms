      ...
      <div className="nms-card space-y-3">
        ...
        {canReinstall && (
          <p className="text-xs text-blue-300 bg-blue-500/10 border border-blue-500/20 rounded px-3 py-2">
            A newer osmo-epdg/strongswan-epdg build is available
            {status?.builtWithOsmoEpdgTag !== status?.currentOsmoEpdgTag ? (
              <> (currently built with{' '}
              {status?.builtWithOsmoEpdgTag ? `v${status.builtWithOsmoEpdgTag}` : 'an older, unrecorded version'}, this
              server is pinned to v{status?.currentOsmoEpdgTag})</>
            ) : (
              <> — same v{status?.currentOsmoEpdgTag} upstream source, but a local bug-fix patch has been added since
              this was last built</>
            )}. Rebuilding only recompiles the binaries — your existing configuration (SMF peer, DNS zone, dummy
            interface) is left alone and does not need to be redone. Remember to Restart the services afterward to
            load the new build.
          </p>
        )}
          ...
          <div className="flex items-start gap-3">
            ...
            <p className="text-xs text-blue-200 leading-relaxed">
              ...
              {status.builtWithOsmoEpdgTag !== status.currentOsmoEpdgTag ? (
                ...
                but this server is now pinned to v{status.currentOsmoEpdgTag}.</>
              ) : (
                <>This deployment is built from the same v{status.currentOsmoEpdgTag} upstream source, but a local
                bug-fix patch has been added since it was last built.</>
              )}{' '}
              Go to the Setup Wizard and click
