...
function kamailioUnit(name: string, cfgFile: string, description: string): string {
  return `[Unit]
...
ExecStart=/usr/sbin/kamailio -f ${cfgFile} -DD -E -e
Restart=on-failure
RestartSec=5
...
After=network.target pyhss-api.service
Requires=pyhss-api.service
...
export function createImsRouter(
  ...
  router.post('/install', requireAdmin, async (req: Request, res: Response) => {
    ...
    await spawnStream(
      ...
      'grep -qr "deb-src" /etc/apt/sources.list /etc/apt/sources.list.d/ 2>/dev/null || ' +
      '(sed "s/^deb /deb-src /" /etc/apt/sources.list > /etc/apt/sources.list.d/deb-src-ims.list && apt-get update -q)\n' +
  ...
  router.post('/configure', requireAdmin, async (req: Request, res: Response) => {
    ...
    try {
      ...
      // 5. daemon-reload (systemd units already exist)
      await nsenter('systemctl', ['daemon-reload']);

      // 6a. PyHSS config — deployed to /opt/pyhss/config.yaml + iFC XMLs
      ...
      // Restart pyhss services to pick up new config
      await nsenter('systemctl', ['restart', 'pyhss-api']).catch(() => {});
      await new Promise(r => setTimeout(r, 3000));
      await nsenter('systemctl', ['restart', 'pyhss-diameter']).catch(() => {});
      await nsenter('systemctl', ['restart', 'pyhss-hss']).catch(() => {});
  ...
  router.post('/sync-subscribers', requireAdmin, async (req: Request, res: Response) => {
    ...
    try {
      ...
      try {
        ...
        // Give pyHSS a moment to re-establish Diameter connections
        await new Promise(r => setTimeout(r, 3000));
      } catch (e) {
