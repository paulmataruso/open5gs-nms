...
|--------|--------------------|-------|
| **IMS / VoLTE** | `kamailio kamailio-ims-modules kamailio-mysql-modules kamailio-tls-modules kamailio-extra-modules kamailio-utils-modules rtpengine mariadb-server bind9 bind9utils mariadb-client dnsutils git dpkg-dev libxml2-dev redis-server python3-pip python3-venv python3-dev` | The Diameter HSS is [PyHSS](https://github.com/nickvsnetworking/pyhss) — **installed automatically by the NMS** (cloned from GitHub, Python deps installed via pip), no separate manual install or JRE/JDK needed. `rtpengine` is only in Ubuntu's own repos from 23.04 onward — on 22.04 the NMS automatically falls back to the `ppa:davidlublink/rtpengine` PPA, no manual step needed. |
